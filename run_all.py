...
def extract_hr_founder_emails(companies):
    ...
    print("[PHASE 2] Extracting HR & Founder emails...")
    print("="*70)
    ...
    for i, company in enumerate(companies, 1):
        try:
            ...
            total = len(hr_emails) + len(founder_emails)
            found_count = len(found_emails.get("hr", [])) + len(found_emails.get("founder", []))
            gen_count = total - found_count
            
            if found_count > 0:
                print(f" [OK - {found_count} found, {gen_count} generated]")
            else:
                print(f" [GENERATED - {gen_count} patterns]")
            
        except Exception as e:
            print(f" [ERROR]")
            time.sleep(0.5)
    
    print(f"\n✓ Extracted {len(email_raw)} HR/Founder email addresses\n")
